...
// Shaders
import imageFragmentShader from '@/scripts/three/images/fake-3d/shaders/image/fragment.glsl';
import imageVertexShader from '@/scripts/three/images/fake-3d/shaders/image/vertex.glsl';
// Constants
...
export default class ThreeScene {
  ...
  domImageContainers = [...document.querySelectorAll('.three-image-container')];
	...
	imageBaseMaterial = new THREE.ShaderMaterial({
		uniforms:{
			...
			uThreshold: new THREE.Uniform(new THREE.Vector2(20, 20)),
			uDepthMap: new THREE.Uniform(null),
		},
  ...
  setupLoaders() {
    ...
    this.domImageContainers.forEach((container) => {
      const img = container.querySelector('.three-image');
			const mapSrc = img.getAttribute('data-map');
			const imageObject = {
			...
			this.textureLoader.load(mapSrc, (map) => {
				imageObject.map = map;
			});
		})
  }
  ...
  setupObjects() {
      ...
      const planeGeometry = new THREE.PlaneGeometry(img.bounds.width, img.bounds.height, 80, 80);
			...
			img.material.uniforms.uDepthMap.value = img.map;
  ...
  tick() {
    ...
    // Materials
		...
		this.images.forEach((img) => {
			img.material.uniforms.uMouse.value = this.lerpedCurrentMouse;
		});
