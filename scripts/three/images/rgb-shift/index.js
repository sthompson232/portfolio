...
// Shaders
import imageFragmentShader from '@/scripts/three/images/rgb-shift/shaders/image/fragment.glsl';
import imageVertexShader from '@/scripts/three/images/rgb-shift/shaders/image/vertex.glsl';
import postprocessingFragmentShader from '@/shaders/postprocessing/rgb-shift/fragment.glsl';
import postprocessingVertexShader from '@/shaders/postprocessing/rgb-shift/vertex.glsl';
// Constants
...
export default class ThreeScene {
  ...
  setupComposer() {
    ...
    this.composer.addPass(new RenderPass(this.scene, this.camera));
		...
		// Shader pass
		this.rgbShiftShaderPass = new ShaderPass({
			uniforms: {
				tDiffuse: new THREE.Uniform(null),
				uResolution: new THREE.Uniform(new THREE.Vector2(1, this.sizes.height / this.sizes.width)),
				uVelocity: new THREE.Uniform(0),
				uMouse: new THREE.Uniform(this.currentMouse),
			},
			vertexShader: postprocessingVertexShader,
			fragmentShader: postprocessingFragmentShader
		});
		this.rgbShiftShaderPass.renderToScreen = true;

		this.composer.addPass(this.rgbShiftShaderPass);
  ...
  tick() {
    ...
    // Materials
		...
		this.rgbShiftShaderPass.uniforms.uMouse.value = this.lerpedCurrentMouse;
		this.rgbShiftShaderPass.uniforms.uVelocity.value = this.mouseSpeed;
  ...
  onResize() {
    ...
    this.images.forEach((img) => {
      ...
      const planeGeometry = new THREE.PlaneGeometry(img.bounds.width, img.bounds.height, 80, 80);
		...
		this.rgbShiftShaderPass.uniforms.uResolution.value = new THREE.Vector2(1, this.sizes.height / this.sizes.width);
